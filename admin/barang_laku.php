...
if(isset($_GET['tanggal'])){
	$tanggal=mysqli_real_escape_string($mysqli,$_GET['tanggal']);
...
<table class="table">
	...
	if(isset($_GET['tanggal'])){
		$tanggal=mysqli_real_escape_string($mysqli,$_GET['tanggal']);
		$brg=mysqli_query($mysqli,"select * from barang_laku where tanggal like '$tanggal' order by tanggal desc");
	}else{
		$brg=mysqli_query($mysqli,"select * from barang_laku order by tanggal desc");
	}
	$no=1;
					...
					<form action="barang_laku_act.php" method="post">
						...
						<div class="form-group">
							...
							<select class="form-control" name="nama" id="nama">
								...
								while($b=mysqli_fetch_array($brg)){
									?>	
									<option value="<?php echo $b['nama']; ?>"><?php echo $b['nama'].' - '.$b['harga']; ?></option>
									<?php 
								}
								?>
							</select>
